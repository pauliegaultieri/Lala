...
export default function DeleteMutationForm({ mutation }) {
  ...
  async function handleDelete() {
    ...
    try {
      const response = await fetch(`/api/admin/mutations/${mutation.id}`, {
        method: "DELETE",
      });

      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || "Failed to delete mutation");
      }

      router.push("/admin/mutations");
      router.refresh();
    } catch (err) {
      setError(err?.message || "Failed to delete mutation");
      setIsDeleting(false);
    }
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex">
          ...
          <div className="ml-3">
            ...
            <div className="mt-2 text-sm text-red-700 dark:text-red-300">
              <p>You are about to permanently delete this mutation. This action cannot be undone.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-[#E5E7EB] dark:border-slate-700 rounded-[16px] overflow-hidden">
        <div className="p-6">
          <h3 className="text-lg leading-6 font-medium text-gray-900 dark:text-white">Mutation Details</h3>
          <div className="mt-5">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="text-lg font-medium text-gray-900 dark:text-white">{mutation.name}</h4>
                <div className="mt-1 flex items-center gap-2">
                  <span
                    className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                    style={{ backgroundColor: mutation.color, color: "white" }}
                  >
