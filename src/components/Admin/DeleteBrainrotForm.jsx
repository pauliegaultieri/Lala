...
export default function DeleteBrainrotForm({ brainrot }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/brainrots/${brainrot.id}`, {
        method: "DELETE",
      });
      
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || "Failed to delete brainrot");
      }
      ...
      <div className="bg-red-50 dark:bg-red-900/30 rounded-[16px] p-4 border border-red-200/60 dark:border-red-800/40">
        <div className="flex">
          ...
          <div className="ml-3">
            ...
            <div className="mt-2 text-sm text-red-700 dark:text-red-300">
              <p>
                You are about to permanently delete this brainrot. This action cannot be undone.
              </p>
            </div>
          </div>
        </div>
              ...
              <div>
                ...
                <div className="mt-1 flex items-center">
                  <span className="text-sm text-gray-500 dark:text-gray-400">
                    {brainrot.rarity.charAt(0).toUpperCase() + brainrot.rarity.slice(1).replace('-', ' ')}
                  </span>
                  <span className="mx-2 text-gray-500 dark:text-gray-400">•</span>
                  <span className="text-sm text-gray-500 dark:text-gray-400">
                    {brainrot.valueLGC} LGC
                  </span>
                </div>
              </div>
            </div>
