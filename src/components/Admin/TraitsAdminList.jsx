...
export default function TraitsAdminList({ traits = [] }) {
  ...
  const filteredTraits = useMemo(() => {
    ...
    if (q) list = list.filter((t) => normalizeString(t?.name).includes(q));

    if (active !== "all") {
      const isActive = active === "active";
      list = list.filter((t) => Boolean(t?.isActive) === isActive);
    }
      ...
      <div className="mb-6">
        ...
        <div className="mt-3 flex items-center gap-4">
          <div className="ml-auto text-sm font-urbanist text-gray-500 dark:text-gray-400">
            Showing {filteredTraits.length} / {traits.length}
          </div>
        </div>
      </div>
        ...
        {filteredTraits.length > 0 ? (
          <ul className="divide-y divide-gray-200 dark:divide-slate-700">
            {filteredTraits.map((trait) => (
              <li key={trait.id} className="px-6 py-5 hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors">
                <div className="flex items-center justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center mt-1 gap-2">
                      <span
                        ...
                        style={{ backgroundColor: trait?.color || "#4F46E5", color: "white" }}
                      >
                        {Number(trait?.multiplier) || 1}x
                      </span>
                  ...
                  <div className="flex items-center gap-2">
                    <Link
                      href={`/admin/traits/${trait.id}/edit`}
                    ...
                    <Link
                      href={`/admin/traits/${trait.id}/delete`}
          ...
          <div className="py-12 text-center">
            <p className="text-gray-500 dark:text-gray-400">No traits match your filters</p>
          </div>
        )}
      </div>
    </div>
