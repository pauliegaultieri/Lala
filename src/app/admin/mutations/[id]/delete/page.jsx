...
import DeleteMutationForm from "@/components/Admin/DeleteMutationForm";

export default function DeleteMutationPage() {
  ...
  useEffect(() => {
    async function fetchMutation() {
      if (!params?.id) {
        setError("Missing mutation ID");
        setLoading(false);
        return;
      }

      try {
        const response = await fetch(`/api/admin/mutations/${params.id}`);
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch mutation: ${response.statusText}`);
        }
        ...
        setError(err?.message || "Failed to load mutation");
      } finally {
        setLoading(false);
      }
    }
    ...
    return (
      <div className="min-h-screen bg-white dark:bg-slate-950 transition-colors flex items-center justify-center">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-4 text-lg font-medium text-gray-700 dark:text-gray-300">Loading mutation...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-md p-4">
            <div className="flex">
              ...
              <div className="ml-3">
                <h3 className="text-sm font-medium text-red-800 dark:text-red-200">Error Loading Mutation</h3>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 md:px-12 py-12">
        <div className="mb-10">
          ...
          <p className="mt-2 text-sm font-urbanist text-gray-600 dark:text-gray-400">
            Are you sure you want to delete {mutation.name}? This action cannot be undone.
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-[#E5E7EB] dark:border-slate-700 rounded-[20px] overflow-hidden transition-colors">
          <div className="p-6">
            <DeleteMutationForm mutation={mutation} />
          </div>
        </div>
      </div>
    </div>
