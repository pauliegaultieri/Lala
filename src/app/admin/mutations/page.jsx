...
import MutationsAdminList from "@/components/Admin/MutationsAdminList";

export const metadata = {
  title: "Manage Mutations - Admin Dashboard",
  description: "Manage mutation modifiers in the Sabrvalues database",
};

export default async function MutationsAdminPage() {
  await requireAdmin();

  const snapshot = await adminDb.collection("mutations").orderBy("multiplier", "desc").get();
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 transition-colors">
      <div className="max-w-6xl mx-auto px-4 sm:px-8 md:px-12 py-12">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-10">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-pp-mori font-semibold text-gray-900 dark:text-white">
              Manage <span className="text-[#4F46E5] dark:text-[#6366F1]">Mutations</span>
            </h1>
          </div>
        ...
        <MutationsAdminList mutations={mutations} />
      </div>
    </div>
  );
}
