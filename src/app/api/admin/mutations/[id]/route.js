...
export async function GET(request, { params }) {
  try {
    ...
    if (!id) return NextResponse.json({ error: "Missing mutation ID" }, { status: 400 });

    const ref = adminDb.collection("mutations").doc(id);
    const doc = await ref.get();

    if (!doc.exists) return NextResponse.json({ error: "Mutation not found" }, { status: 404 });

    return NextResponse.json({ id: doc.id, ...doc.data() });
  } catch (error) {
    console.error("Error fetching mutation:", error);
    return NextResponse.json({ error: "Failed to fetch mutation" }, { status: 500 });
  }
}
